...
# Tags below are how EKS and the AWS Load Balancer Controller discover
# which subnets to place internet-facing vs internal load balancers in -
# without them, ALB/NLB provisioning from an Ingress silently fails to
# pick a subnet.
...
# Single NAT gateway keeps the demo cheap; a production build would run one
# per AZ so a single AZ outage doesn't take every private subnet's egress
# down with it.
