# 🕵️‍♂️ Tracee in Kubernetes — Runtime Threat Detection Lab (CKS Style)

We’ll do a **full mini-lab**:

1. Deploy Tracee as a **DaemonSet**
2. Deploy a **test pod**
3. Run **“attacks”** from inside the pod (reverse shell / suspicious commands)
...
- A running **Kubernetes cluster** (kind / kubeadm / cloud, doesn’t matter)
- `kubectl` configured
...
## 🧩 2. Deploy Tracee as a DaemonSet

Tracee runs on **every node** to watch all containers. That’s why we use a **DaemonSet**.
...
Leave this **running** in a terminal. This is your **“security camera monitor”**.

---

## 🧪 4. Create a Test Pod to Simulate “Attacker”

Now let’s spawn a pod where we’ll run suspicious commands.
...
## 🔥 5. Scenario 1 — Reverse Shell Detection
...
(You don’t actually need a real listener for this lab; the attempt is enough.)

👉 In the **Tracee logs** terminal, you should see an alert similar to:
...
## 🔥 6. Scenario 2 — Suspicious Syscalls / Possible Escape Attempt
...
## 🔥 7. Scenario 3 — File Tampering in Sensitive Paths

Let’s simulate tampering in `/etc` (in the container’s filesystem):
...
Or if it has a generic file integrity rule, something like **“suspicious file modification”**.

---

## 🎚️ 8. Use Filters to Reduce Noise (Optional but Nice)
...
## 🧠 10. How This Ties Into CKS Exam
...
- **Tracee / Falco** → Runtime _detection_
- **Seccomp / AppArmor / SELinux** → Runtime _prevention_

A good mental model:

> 👀 Tracee = “Security camera” (observe & alert)
> 🛡 Seccomp = “Locked doors” (prevent certain syscalls)

---

## 📝 Tiny Summary (For Your Notes)

- Deploy Tracee as **DaemonSet** in `tracee` namespace
- Watch alerts: `kubectl -n tracee logs -f daemonset/tracee`
- Run “attacks” from a test pod (reverse shell, unshare, mount, file tampering)
- See **high-level alerts** with rule names in Tracee logs
- Remember: Tracee = eBPF-based runtime detection → very CKS-relevant
