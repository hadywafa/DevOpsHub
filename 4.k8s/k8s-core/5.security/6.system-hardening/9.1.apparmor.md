...
> **“A permission whitelist for binaries.”**
>
...
## 🧠 Why AppArmor Exists (Simple Explanation)

Containers share the host kernel → if a container is compromised, attacker tries:
...
AppArmor **prevents** this by applying a **profile** to the container’s process.

---

## 🧩 AppArmor vs. Seccomp (Clear Distinction)

| Feature                | Seccomp | AppArmor                          |
| ---------------------- | ------- | --------------------------------- |
| Controls syscalls      | ✔ Yes   | ✔ Partially (via mediation rules) |
| Controls file paths    | ❌ No   | ✔ Yes                             |
| Controls capabilities  | ❌ No   | ✔ Yes                             |
| Controls network rules | ❌ No   | ✔ Yes                             |
| Blocks kernel modules  | ❌ No   | ✔ Yes                             |
| Difficulty             | Medium  | Easy                              |
| Default in Ubuntu?     | No      | ✔ Yes (AppArmor built-in)         |
...
## 📁 2. AppArmor Profiles (The Core Concept)
...
## ☸️ **Using AppArmor With Kubernetes Pods**
...
## 🧷 Apply AppArmor in Pod YAML

### 📝 Old way using Annotations
...
If the profile does NOT exist on the node → the pod will still run, but AppArmor will NOT be applied.

---

## 🔥 **Demo — Block File Writes With AppArmor (Hands-On)**
...
## 🛠️ **Editing / Creating Profiles Automatically**
...
## 🧨 **How AppArmor Helps in CKS Exam**

You will use AppArmor to:

- ✔ Deny container from writing on host-like paths
- ✔ Block suspicious binaries
- ✔ Restrict container processes
- ✔ Combine AppArmor + Seccomp for maximum hardening
- ✔ Demonstrate knowledge of runtime restrictions
...
> “Apply AppArmor to block write access for a pod.”
...
👉 **AppArmor vs Seccomp vs SELinux** (exam comparison table)  
👉 **Full hands-on: build profile from scratch**  
👉 **CKS exam-style AppArmor challenge**
