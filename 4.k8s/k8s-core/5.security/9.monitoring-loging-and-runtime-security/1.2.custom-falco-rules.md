# 🔧 **Configuring Custom Falco Rules**
...
## 🟧 **Scenario 1 — Falco Installed as a System Service (Host / Bare Metal / VM)**

💡 _Falco installed via apt/yum, acting as a kernel-level agent outside Kubernetes._
...
### ✅ **Add Custom Rules to System Installation**
...
## 🟦 **Scenario 2 — Falco Installed via Helm (Kubernetes DaemonSet)**

💡 _Most common production installation_
...
### ✅ **Option A — Add Custom Rules Using Helm values.yaml (Recommended)**
...
### ✅ **Option B — Manually Create a ConfigMap and Patch DaemonSet**

If you don’t want to modify Helm values:
...
### 3. **Rules** – actual alerts you care about
...
Falco evaluates:
**event → check condition → if true → emit output at priority level.**

---

## 🔖 **Common Fields & Conditions You’ll Use**
...
| `evt.type`            | Syscall type (`execve`, `open`, `write`, `connect`, …) |
...
## 📝 **Real Examples** - Writing Custom Falco Rules –
...
You’d usually put these into a file like `custom-rules.yaml`.
...
### 🌐 Rule 2 – Detect curl/wget in containers (data exfiltration)
...
### 🧾 Rule 3 – Write to `/etc/passwd` or `/etc/shadow`
...
Any modification to these files is almost always malicious unless you’re in a very specific maintenance flow.

---

### ☸️ Rule 4 – Detect `kubectl exec` into pods

Falco can also watch Kubernetes metadata (through the audit logs or via kubelet/syscalls depending on deployment) – but a simple approach is to detect `kubectl` running with `exec`.
...
You don’t necessarily block it, but you **log every occurrence**.

---

## 🧪 **Test & Debug Your Rules**
...
If rule is correct, you’ll see alerts in:
...
- **Macros** → reusable snippets of conditions (“in container”, “writing file”)
- **Rules** → actual security logic + output
