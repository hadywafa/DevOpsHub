# 🔥 **Linux File Comparison & Deduplication Tools**

Today we cover two powerful tools:

- **`uniq`** → remove duplicate _lines_
- **`diff`** → compare two files (or directories)

Used heavily in:

- Log processing
- Sorting & data analysis
- DevOps CI/CD validation
- Config comparison
- Git conflict debugging
- Backup verification

Let’s go deep 🚀

---

## 🟨 1. `uniq` — Remove Duplicate Lines (Very Useful After `sort`)

`uniq` removes duplicate **consecutive** repeated lines.

📌 **Important:** It only removes duplicates that are next to each other.
That’s why we often do:

```bash
sort file.txt | uniq
```

---

### 🧩 1.1 Basic Syntax

```bash
uniq file
uniq input.txt output.txt
```

---

### 🟦 1.2 Basic Usage

Given a file:

```ini
apple
apple
banana
banana
banana
orange
```

#### ▶️ Remove duplicates:

```bash
uniq fruits.txt
```

📤 Output:

```ini
apple
banana
orange
```

---

### 🟩 1.3 Count duplicates (`-c`)

```bash
uniq -c fruits.txt
```

📤 Output:

```ini
  2 apple
  3 banana
  1 orange
```

---

### 🟪 1.4 Show ONLY duplicate lines (`-d`)

```bash
uniq -d fruits.txt
```

📤 Output:

```ini
apple
banana
```

---

### 🟫 1.5 Show ONLY unique (non-duplicate) lines (`-u`)

```bash
uniq -u fruits.txt
```

📤 Output:

```ini
orange
```

(because only `orange` appears once)

---

### 🟧 1.6 Case-insensitive (`-i`)

```bash
uniq -i file.txt
```

---

### 🟥 1.7 Filter duplicate words after `sort`

Example file:

```ini
error
info
error
warning
info
```

#### Without sorting:

```bash
uniq file.txt
```

Output: ❌ incorrect

```ini
error
info
error
warning
info
```

#### With sorting:

```bash
sort file.txt | uniq
```

Correct output:

```ini
error
info
warning
```

---

### 🧠 Real DevOps Use: Count Unique IPs in Logs

```bash
awk '{print $1}' access.log | sort | uniq -c
```

---

## 🟨 2. `diff` — Compare Files (Line by Line)

`diff` shows differences between **two files** or **two directories**.

---

### 🧩 2.1 Basic Syntax

```bash
diff file1 file2
```

---

### 🟦 2.2 Output Example

File1:

```ini
apple
banana
orange
```

File2:

```ini
apple
banana
grape
orange
```

#### ▶️ Run:

```bash
diff file1.txt file2.txt
```

📤 Output:

```ini
3c3
< orange
---
> grape
```

Meaning:

- Line 3 changed
- `<` shows old value
- `>` shows new value

---

### 🟥 2.3 Symbols in diff

| Symbol | Meaning         |
| ------ | --------------- |
| `a`    | added           |
| `c`    | changed         |
| `d`    | deleted         |
| `<`    | line from file1 |
| `>`    | line from file2 |

---

### 🟩 2.4 Unified Diff Format (`-u`) — Git-style

The most common and readable format:

```bash
diff -u file1.txt file2.txt
```

📤 Output:

```ini
--- file1.txt
+++ file2.txt
@@ -1,3 +1,4 @@
 apple
 banana
-orange
+grape
 orange
```

This is the SAME output style used by Git.

---

### 🟪 2.5 Side-by-side Comparison (`-y`)

```bash
diff -y file1.txt file2.txt
```

📤 Output:

```ini
apple     apple
banana    banana
orange  | grape
         orange
```

`|` highlights differences.

---

### 🟫 2.6 Suppress identical lines (`--suppress-common-lines`)

```bash
diff -y --suppress-common-lines file1 file2
```

Only differences appear.

---

### 🟧 2.7 Compare Directories (recursive)

```bash
diff -r dir1 dir2
```

Useful for checking config drift.

---

### 🟨 2.8 Ignore whitespace changes (`-w`)

```bash
diff -w file1 file2
```

Great for comparing code with formatting differences.

---

### 🟥 2.9 Ignore case

```bash
diff -i file1 file2
```

---

### 🧠 2.10 Real DevOps Examples

#### ✔ Compare two versions of a config file

```bash
diff -u config.old config.new
```

---

#### ✔ Check differences between two deployment templates

```bash
diff -r deploy-v1/ deploy-v2/
```

---

#### ✔ Verify only a few lines changed

```bash
diff -uN old new
```

`-N` treats missing files as empty.

---

#### ✔ Compare K8s manifest before and after CI pipeline modification

```bash
diff -u <(kubectl get deploy myapp -o yaml) new.yaml
```

---

#### ✔ Compare secrets in a secure environment

```bash
diff <(echo "$OLD_SECRET") <(echo "$NEW_SECRET")
```

---

## 🧩 Summary Cheat Sheet

| Tool      | Purpose                | Example                 |     |
| --------- | ---------------------- | ----------------------- | --- |
| `uniq`    | Remove duplicate lines | `sort users.txt | uniq` |
| `uniq -c` | Count duplicates       | `uniq -c items.txt`     |     |
| `uniq -d` | Show only duplicates   | `uniq -d items.txt`     |     |
| `uniq -u` | Unique lines only      | `uniq -u items.txt`     |     |
| `diff`    | Compare files          | `diff f1 f2`            |     |
| `diff -u` | Unified diff           | `diff -u old new`       |     |
| `diff -y` | Side-by-side diff      | `diff -y a b`           |     |
| `diff -r` | Compare directories    | `diff -r dir1 dir2`     |     |
| `diff -w` | Ignore whitespace      | `diff -w code1 code2`   |     |
