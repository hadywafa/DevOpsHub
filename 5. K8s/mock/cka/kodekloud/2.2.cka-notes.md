...
```bash
kubectl run test --rm -i --restart=Never --image=busybox:1.28  -- nslookup <service-name> > /root/CKA/nginx.svc
```
...
```bash
kubectl run test --rm -i --restart=Never --image=busybox:1.28  -- nslookup <P-O-D-I-P.default.pod> > /root/CKA/nginx.pod
```
...
![alt text](events-troubleshooting.png)
