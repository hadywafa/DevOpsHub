...
<div align="center" style="background-color:#F3F3F3; border-radius: 10px; border: 2px solid">
  <img src="image/8.3.seccomp/1763291946824.png" alt="Mock Exam 1 Question 1" style="width: 80%">
</div>
...
#### 2. Run the container with your profile
...
#### 3. Install a tool that uses `ptrace`

Inside the container, install `strace` (it relies on `ptrace` internally):
...
#### 4. Try to trace a process

Run `strace` on something simple:

```bash
strace ls
```

- **Expected result:** It should fail with `Operation not permitted` because `ptrace` is blocked by your profile.
- Without seccomp, `strace ls` would succeed and print syscall traces.
...
### 👉🏻 Using Pod annotations (old method)
...
### 👉🏻 Using securityContext (NEW and recommended)
