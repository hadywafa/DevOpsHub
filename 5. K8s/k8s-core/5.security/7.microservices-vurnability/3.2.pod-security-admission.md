...
<div align="center" style="background-color:#ECECEC; border-radius: 10px; border: 2px solid">
  <img src="image/1763375379359.png" alt="OpenSSL" style="width: 90%">
</div>

---

<div align="center" style="background-color:#ECECEC; border-radius: 10px; border: 2px solid">
  <img src="image/1763375293256.png" alt="OpenSSL" style="width: 90%">
</div>

---

**PSA (Pod Security Admission) is enabled by default in modern Kubernetes (v1.25 and later). You do not need to add it manually to `--enable-admission-plugins` in the kube-apiserver manifest.**
...
- **PodSecurityAdmission (PSA)** replaced the older **PodSecurityPolicy (PSP)**, which was deprecated and removed in Kubernetes v1.25.
- Starting with **Kubernetes v1.25**, PSA is a **built-in admission controller** that is **enabled by default**. This means:
  - You don’t need to explicitly list it in `--enable-admission-plugins`.
  - It automatically enforces Pod Security Standards (Privileged, Baseline, Restricted) based on **namespace labels**.
- If you’re running a **managed Kubernetes service** (like AKS, EKS, GKE), PSA is also enabled by default. For example, AKS explicitly states PSA is on by default.

---

- You configure PSA enforcement by **namespace labels**, for example:
...
This enforces the **Restricted** profile, audits against **Baseline**, and warns on violations.
