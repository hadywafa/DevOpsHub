# 🗂️ **GitHub Actions: File & Repo Structure**

```ini
<repo-root>/
├─ .github/
│  ├─ workflows/                  # All workflow YAMLs live here
│  │  ├─ ci.yml
│  │  └─ release.yml
│  ├─ actions/                    # (Optional) Local actions you author
│  │  └─ my-composite-action/
│  │     ├─ action.yaml           # Required manifest for any action
│  │     └─ README.md
│  ├─ ISSUE_TEMPLATE/             # Not actions, but often nearby
...
## 🧬 **Workflow YAML Schema** (top-level keys)

> A workflow file is a YAML document with these common top-level keys:

- `name: string` – Human-friendly workflow name.
- `run-name: string with expressions` – Per-run dynamic name.
  Example: `run-name: "Build #${{ github.run_number }} for ${{ github.ref_name }}"`
- `on:` – **Triggers** (events) that start the workflow.
- `permissions:` – Fine-grained default token scopes for this workflow.
- `env:` – Global env vars available to all jobs/steps (can be overridden).
- `defaults:` – Default `run` options (shell, working-directory) for all steps.
- `concurrency:` – Prevents overlapping runs of the same group; supports cancelation.
- `jobs:` – One or more jobs to execute (possibly in parallel).

---

<div align="center" style="background-color:#fff; border-radius: 10px; border: 2px solid">
   <img src="image/1762687368642.png" alt="GitHub Actions Matrix Build" style="width: 80%">
</div>

---

### ✅ Full list of commonly used top-level keys & value shapes

```yaml
name: "string"
run-name: "string with ${{ expressions }}"
on:
  # see the “Events & Filters” section below
...
## 🧰 **Reusable Workflows** (`workflow_call`) & How to Invoke
...
## 🪝 **Expression Language & Contexts** (the brain of conditions)

Use `${{ ... }}` for expressions in keys like `if:`, `run-name:`, `with:`, etc.

**Common contexts:**

- `github` – event metadata (`github.event`, `github.ref`, `github.actor`, `github.sha`, `github.ref_name`, …)
- `env` – current environment variables
- `secrets` – repository/organization/environment secrets
- `vars` – repository/organization/environment **variables** (not secrets)
- `inputs` – `workflow_dispatch`/`workflow_call` inputs
- `runner` – runner info (`os`, `arch`, `temp`, `workspace`…)
- `matrix` – strategy matrix values
- `jobs`, `steps` – job/step-level metadata
...
- `hashFiles(glob)` – deterministic hash for file set
...
run-name: "Publish ${{ github.ref_name }} • build #${{ github.run_number }}"
```

---

## 🔐 **Secrets**, **Variables**, and **Environments** (how values flow)

- **Secrets**: `${{ secrets.MY_TOKEN }}` – masked in logs; set at repo/org/environment level.
- **Variables**: `${{ vars.MY_FLAG }}` – non-secret config; set at repo/org/environment.
...
## 🧱 **Local (Composite) Actions** (reusable building blocks inside your repo)
...
## ✍🏻 **Typical CI Workflow** (end-to-end example)

```yaml
# .github/workflows/ci.yml
name: "CI"
run-name: "CI • ${{ github.ref_name }} #${{ github.run_number }}"
on:
  push:
    branches: [main]
    paths-ignore: ["docs/**"]
...
## 🧯 **Caching, Artifacts**, & Common Building Blocks
...
## 🔒 **Permissions Best Practice** (least privilege)
...
## 🧱 **Self-Hosted Runners** (label matching)
...
## 🧪 **Quick Reference**: Common “All Possible” Values (cheat sheet)
...
- `with:` inputs map
- `if:` expression; `env:` map; `shell:` …; `id:` string; `timeout-minutes:` number
